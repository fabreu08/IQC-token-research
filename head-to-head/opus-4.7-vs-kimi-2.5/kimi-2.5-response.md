...
The documents should stop treating this as an architectural mystery. It's a straightforward implementation choice.
...
**PoC Requirements:**
- Every Critical/High finding must have a runnable test demonstrating the exploit or failure mode
- "Potential" issues without PoCs should be Informational at highest
- Reentrancy claims require a test with a malicious reentrant contract

**Severity Justification Framework:**
- **Critical**: Unconditional fund loss, systemic failure, or attacker extraction of protocol value
- **High**: Conditional fund loss (requires specific user action or privileged compromise), significant economic manipulation
- **Medium**: Broken invariants without direct fund loss, observability issues, gas griefing
- **Low/Informational**: Design suggestions, gas optimizations, documentation improvements
...
## 5. Prioritization for Mainnet: True Blockers
...
Either switch to `_burn()` via Registry self-burn (Option A above) or explicitly document that burns don't reduce supply. The current situation—claiming burns happen while supply never decreases—is misleading.

**P0-4: Protect Allocation Contracts**
Add a check in `TokenAllocation.recoverERC20` that prevents recovering IQC specifically, or remove the function for IQC. The current implementation allows owner to drain purpose-bound allocations, which defeats the transparency model.
